...
import { getSavedPlanbookCollectionsByTeacherId, createPlanbookCollection, deletePlanbookCollection, updatePlanbookCollection, Collection } from '@/data/teacher/CollectionData';
import { getUserId } from '@/data/apiClient';

const { Option } = Select;

const SavedCollectionPage: React.FC = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const data = await getSavedPlanbookCollectionsByTeacherId(getUserId()!);
                setCollections(data);
            } catch (error) {
    ...
    const handleItemClick = (collectionId: string) => {
        navigate(`/teacher/saved-collection/${collectionId}`);
    };
    ...
    return (
        <div className="p-6 bg-gray-100 min-h-screen">
            <h1 className="text-2xl font-semibold mb-4 text-center">Bộ sưu tập lưu</h1>
